 ...
 */
class wpgpo_AdminFormRenderer {
	function _get_option () {
		return WP_NETWORK_ADMIN ? get_site_option('wpgpo') : get_option('wpgpo');
	}
	...
	function _create_checkbox ($name) {
		...
		$value = @$opt[$name];
		return
			"<input type='radio' name='wpgpo[{$name}]' id='{$name}-yes' value='1' " . ((int)$value ? 'checked="checked" ' : '') . " /> " .
				"<label for='{$name}-yes'>" . __('Yes', 'wpgpo') . "</label>" .
			'&nbsp;' .
			"<input type='radio' name='wpgpo[{$name}]' id='{$name}-no' value='0' " . (!(int)$value ? 'checked="checked" ' : '') . " /> " .
				"<label for='{$name}-no'>" . __('No', 'wpgpo') . "</label>" .
		"";
	}
	...
	function _create_radiobox ($name, $value) {
		...
		return "<input type='radio' name='wpgpo[{$name}]' id='{$name}-{$value}' value='{$value}' " . ($checked ? 'checked="checked" ' : '') . " /> ";
	}
	...
	function create_appearance_box () {
		...
		foreach ($appearances as $pos => $label) {
			$img = "<br /><img src='" . wpgpo_PLUGIN_URL . "/img/{$pos}.png' /> <img src='" . wpgpo_PLUGIN_URL . "/img/{$pos}-count.png' />";
	...
	function create_position_box () {
		$positions = array (
			'top' => __('Before the post', 'wpgpo'),
			'bottom' => __('After the post', 'wpgpo'),
			'both' => __('Both before and after the post', 'wpgpo'),
			'manual' => __('Manually position the box using shortcode or widget', 'wpgpo'),
		);
	...
	function create_language_box () {
		...
		$opt = $this->_get_option();
		echo "<select name='wpgpo[language]'>";
	...
	function create_skip_post_types_box () {
		...
		foreach ($post_types as $tid=>$type) {
			...
			echo
				"<input type='hidden' name='wpgpo[skip_post_types][{$type}]' value='0' />" . // Override for checkbox
				"<input {$checked} type='checkbox' name='wpgpo[skip_post_types][{$type}]' id='skip_post_types-{$tid}' value='{$type}' /> " .
